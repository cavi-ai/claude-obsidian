...
- **API key only, no OAuth.** Companion talks to the Anthropic Messages API with the user's API key. Claude.ai Free/Pro/Max OAuth tokens are deliberately *not* supported (policy). Don't add subscription-OAuth auth paths.
