...
export function fromResearchProject(record: ResearchProjectRecord): ChatProject {
  const folder = record.path.endsWith("/Project.md") ? record.path.slice(0, -"/Project.md".length) : null;
  return { id: record.path, name: record.title, folder, pinned: [], instructions: record.question, source: "research" };
}
