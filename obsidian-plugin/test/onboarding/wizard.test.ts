...
import { wizardPlan, WIZARD_DISMISSED_SETTINGS, type WizardState } from "../../src/onboarding/wizard";
...
describe("wizardPlan", () => {
  ...
  it("includes step 1 iff there is no credential", () => {
    expect(wizardPlan(state({ needsCredential: true }))).toEqual(["connect"]);
    expect(wizardPlan(state({ needsCredential: false }))).toEqual([]);
  });
  ...
  it("dismissing without a credential leaves needsCredential true and the plan non-empty next launch", () => {
    // Dismissal only ever persists WIZARD_DISMISSED_SETTINGS — it cannot
    // clear needsCredential, so a fresh plan computed from the same
    ...
    expect(wizardPlan(dismissedState)).toEqual(["connect"]);
  });
});
