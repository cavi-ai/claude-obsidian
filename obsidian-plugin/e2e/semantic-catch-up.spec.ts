...
test("notes that arrived while the plugin was off are indexed at startup without a manual rebuild", async () => {
  const harness = await launchObsidianHarness({ embedStub: true });
  const { page } = harness;
  try {
