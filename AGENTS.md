...
- **`semantic/`** — the local-embeddings index behind semantic search and related notes: `indexer.ts` orchestrates traverse → chunk → embed → store (IO injected; embeddings via the built-in transformers.js worker (default, cross-platform) or Ollama; index build runs on desktop and mobile); `pdf.ts` extracts and page-locator-chunks vault PDFs (pdf.js bundled via `pdfjs.ts` with its own inlined worker); `store.ts` is the pure vector store + (de)serialization.
- **`web/`** — the agent's web tools: `search.ts` (DuckDuckGo HTML keyless + Brave keyed adapters behind one result shape) and `fetch.ts` (one page → bounded readable markdown via Defuddle). Pure, IO injected; gated by `webSearchEnabled`/`webFetchEnabled`.
- **`mcp/`** — bridge: `server.ts` is the loopback HTTP MCP server (binds `127.0.0.1` only, bearer-token auth); `vaultTools.ts` implements read tools always (`vault_search`, `note_read`, `list_recent`, `vault_tags`, `list_titles`, `get_backlinks`, `get_outgoing_links`, `frontmatter_query`) and write tools when `mcpAllowWrites` (`note_create`, `note_append`, `note_update`, `update_frontmatter`, `note_move`, `base_create`, `canvas_create`); `protocol.ts` is the JSON-RPC framing; `clientConfig.ts` emits the paste-ready Claude Code / Desktop connection snippets. Client (external servers in agent mode): `client.ts` (pure session), `external.ts` (`mcp__<server>__<tool>` namespacing), `httpTransport.ts` (streamable HTTP incl. SSE), `stdioTransport.ts` (desktop), `externalManager.ts` (lazy connect + routing, `mcpClientServers` setting).
...
- **`edit/rewrite.ts`** — the inline-rewrite model: presets, prompt builders (plain + grounded), and response normalization for the chat-free "Rewrite selection with Claude" editor flow (`view/RewriteModal.ts` + `main.ts:runInlineRewrite`) and the Research Desk grounded helpers. Pure.
- **`research/`** — the evidence-backed research system behind the Research Desk/Workbench: `repository.ts` (canonical markdown records: projects, sources, evidence, claims, documents), `graph.ts` (project snapshot), `audit.ts` (structural findings), `draftCoordinator.ts`/`revisionCoordinator.ts` (claim-grounded section drafting + claim-preserving revision), `deskViewModel.ts` (deterministic next-action guidance), `triage.ts` (one-click clippings → themed board), `interpretation.ts` (evidence Interpretation-block upsert). All pure with IO injected.
- **`discovery/`** — scholarly discovery (OpenAlex/Crossref/arXiv/Zotero adapters) with provenance, deterministic + model reranking, and deliberate import into the research library. External requests only fire on explicit Search.
...
`esbuild.config.mjs` bundles `src/main.ts` → `main.js` (CJS, es2021) in three passes: pass 1 bundles `src/semantic/transformers/worker.ts` (transformers.js included) to `.build/embed-worker.txt`; pass 1b bundles the pdf.js worker to `.build/pdf-worker.txt`; pass 2 inlines both files as text (`loader: { ".txt": "text" }`) into `main.js`, which instantiates them via Blob-URL workers at runtime. `obsidian`, `electron`, CodeMirror packages, and all Node builtins are marked **external** — the MCP server uses Node `http`, which exists at runtime on desktop because Obsidian's Electron exposes Node. Desktop-only features are now just the MCP bridge and session import, gated at runtime; chat, artifacts, and semantic search all run on mobile too.
